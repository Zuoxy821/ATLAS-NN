...
n_train, neurons, epochs, lr = N, 80, int(5e4), 3e-3
...
# (1) y-x
plt.subplot(3, 2, 1)
plt.plot(x_num, y_num, '-g', linewidth=lineW, label='Ground truth (SciPy)')
plt.plot(x_s, y_s, ':k', linewidth=lineW, label='Euler')
plt.plot(x_exp, y_exp, '-.b', linewidth=lineW, label=r'$NN \  f(t)=1-e^{-t}$')
plt.plot(x_tanh, y_tanh, '-.m', linewidth=lineW, label=r'$NN \ f(t)=\tanh(mt)$')

plt.plot(x_v2, y_v2, '-.r', linewidth=lineW, label=r'$NN \  f(t)=\frac{1-e^{-\alpha t}}{1+\beta e^{-\alpha t}}$')
plt.ylabel('y'); plt.xlabel('x')

# (2) Energy E(t)
plt.subplot(3, 2, 2)
plt.plot(t_num, E_ex, '-g', linewidth=lineW, label='Ground truth (SciPy)')
plt.plot(t_s, E_s, ':k', linewidth=lineW, label='Euler')
...
plt.ylabel(''); plt.xlabel('t')
plt.ylim([0.1275,0.129])
plt.xlim([0, np.pi*6])
x_ticks = np.arange(0, 6*np.pi+0.001, np.pi)
plt.gca().set_xticklabels(['0' if round(x, 1) == 0.0 else f'{x/np.pi:.0f}π' if x in x_ticks else '' for x in x_ticks])
plt.xticks(x_ticks)
yticks_pos = [0.995 * E0, E0, 1.005 * E0]
# 定义要显示的自定义标签
yticks_labels = [f"0.995E0", f"E0", f"1.005E0"]
# 应用自定义刻度和标签（只显示这三个标签，保留坐标轴原始尺度）
plt.yticks(ticks=yticks_pos, labels=yticks_labels)
plt.legend(loc='lower right')
#  x-t  y-t
plt.subplot(3, 2, 3)
plt.plot(t_num, x_num, '-g', linewidth=lineW, label='Ground truth (SciPy)')
plt.plot(t_s, x_s, ':k', linewidth=lineW, label='Euler')
...
plt.plot(t_s, y_s, ':k', linewidth=lineW, label='Euler')
plt.plot(t_net, y_exp, '-.b', linewidth=lineW, label=r'$NN \  f(t)=1-e^{-t}$')
plt.plot(t_net, y_tanh, '-.m', linewidth=lineW, label=r'$NN \ f(t)=\tanh(mt)$')
...
plt.plot(t_s, px_s, ':k', linewidth=lineW, label='Euler')
...
plt.plot(t_s, py_s, ':k', linewidth=lineW, label='Euler')
plt.plot(t_net, py_exp, '-.b', linewidth=lineW, label=r'$NN \  f(t)=1-e^{-t}$')
plt.plot(t_net, py_tanh, '-.m', linewidth=lineW, label=r'$NN \ f(t)=\tanh(mt)$')
plt.plot(t_net, py_v2, '-.r', linewidth=lineW, label=r'$NN \  f(t)=\frac{1-e^{-\alpha t}}{1+\beta e^{-\alpha t}}$')
plt.xlim([0, np.pi*6])
x_ticks = np.arange(0, 6*np.pi+0.001, np.pi)
plt.gca().set_xticklabels(['0' if round(x, 1) == 0.0 else f'{x/np.pi:.0f}π' if x in x_ticks else '' for x in x_ticks])
plt.xticks(x_ticks)
plt.ylabel('$p_y$'); plt.xlabel('t')
plt.savefig('HenonHeiles_trajectories.pdf')
...
# ===============================
# save all data into one .mat
# ===============================
savemat(os.path.join(mat_dir, "HH_v2.mat"), {
